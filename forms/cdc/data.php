<?php
	if(isset($_GET['pass'])){
		...
		.duplicated{background:yellow}
			...
			<?php
				// select data
				$stmt = $db->prepare("SELECT id,ticket_id,name,faculty,grade,university,email,phone,facebook,comments,date,status,transportation,nid FROM cdc ORDER BY status ASC");
				$stmt->execute();
				$rows = $stmt->fetchAll();
				foreach($rows as $row){
					extract($row);
			...
			?>
					<tr id="<?php echo $id; ?>" class="table-row <?php if($status == '1'){echo 'table-success';}else if($status == '2'){echo 'table-danger';}?>">
					  ...
					  <td class="pho"><?php echo $phone; ?></td>
...
<script>
	...
	// check for duplicated phones
	var phones = Array();
	$(".pho").each(function(){
		let pho = $(this).text();
		if(phones.indexOf(pho) >= 0){
			// phone duplicated
