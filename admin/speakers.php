<?php
	...
	*/
	...
	if(isset($_SESSION['user_type'])){
		...
		if($do == 'Manage'){
		?>
			<h1 class="text-center mb-4 mt-4">Manage Speakers</h1>
			<a href="speakers.php?do=add"><div class="add-button position-fixed d-flex justify-content-center align-articles-center">+</div></a>
		<?php
		// getting users data
		$stmt = $db->prepare("SELECT id,name,position,description,image FROM speakers");
		$stmt->execute();
		if($stmt->rowCount() > 0){
				// there is users
				$rows = $stmt->fetchAll();
			...
			?>
				<tr>
				  ...
				  <td>
					...
					<a href="speakers.php?do=edit&item_id=<?php echo $row['id']; ?>" class="mb-2 btn btn-info btn-sm"><i class="fas fa-edit"></i> Edit</a>
					<a href="speakers.php?do=delete&item_id=<?php echo $row['id']; ?>" class="mb-2 btn btn-danger confirm btn-sm"><i class="fas fa-trash-alt"></i> DELETE</a>
			...
			if(isset($_SERVER['HTTP_REFERER'])){
				...
				$count = checkItem('speakers' , 'id' , $item_id );
				if($count > 0){
					$stmt = $db->prepare("DELETE FROM speakers WHERE id = ?");
		...
		?>
		<h1 class="text-center mt-4">Add New speaker</h1>
			<form action="speakers.php?do=insert" enctype="multipart/form-data" method="POST"class="new-item-form">
			
				<div class="form-group row">
					<label for="userEmail" class="col-sm-2">speaker name</label>
				...
				<div class="form-group row">
					<label for="userEmail" class="col-sm-2">speaker position</label>
					<div  class="col-sm-10  col-md-8 position-relative">
						<!-- will be showing real time results with ajax -->
						<input name="position" type="text" placeholder="position..."  class="form-control" id="userEmail" required="">
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				$dir = scandir(__DIR__ . '/../');
					...
					if(count($errors) == 0){
						...
						$stmt = $db->prepare("INSERT INTO speakers (name,position,description,image) VALUES (?,?,?,?)");
						$stmt->execute(array($name ,$position ,$description , $image_name));
				...
				$stmt = $db->prepare("SELECT id,name,position,description FROM speakers WHERE id = ?");
				$stmt->execute(array($item_id));
				...
				<h1 class="text-center mb-4 mt-4">Edit Speaker</h1>
					
					<form action="speakers.php?do=update" enctype="multipart/form-data" method="POST"class="new-item-form">
					...
					<div class="form-group row">
						<label for="userEmails" class="col-sm-2">Position</label>
						<div  class="col-sm-10  col-md-8">
							<input name="position" type="text" placeholder="position..."  class="form-control" id="userEmails" required="" value="<?php echo $position; ?>">
						</div>
					</div>
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				$dir = scandir(__DIR__ . '/../');
					...
					if(count($errors) == 0){
						...
						if($fileerror == 4){
							// no photo updated
							$stmt = $db->prepare("UPDATE speakers SET name = ?,position =? , description =? WHERE id = ?");
							$stmt->execute(array($name,$position , $description , $item_id));
						}else if($filesize > 0){
						...
						if($filetype == 'image/jpeg' || $filetype == 'image/jpg'){
							...
							$stmt = $db->prepare("UPDATE speakers SET name = ?,position =? , description =? ,image = ? WHERE id = ?");
							$stmt->execute(array($name,$position , $description , $image_name , $item_id));
						}
						
						$msg = '<div class="alert alert-success text-center">Item Updated Successfully</div>';
						redirect('speakers.php' , $msg , 5);
